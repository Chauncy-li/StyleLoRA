...
import sys
# 添加 project_a 的路径到 sys.path
project_a_path = "/home/lsw/meta_programs/mdsn/scenarionet"
...
def write_to_directory(
    ...
    **kwargs
    ...
    """调用了 write_to_directory_single_worker 函数，其为关键的处理函数"""
...
def write_to_directory_single_worker(
    ...
    **kwargs
    ...
    for scenario in scenarios:
        ...
        # 使用循环计数器 `count` 来生成唯一的文件名
        # 这可以防止因 scenario_id 重复而导致的文件覆盖问题。
        # 新的文件名将是 "dataset_name-version-0.pkl", "dataset_name-version-1.pkl", ...
        export_file_name = SD.get_export_file_name(dataset_name, dataset_version, str(count))

        # # [在此处注释掉] 绕过对新版 MetaDrive 的依赖检查。
        # # 摘要信息可以在后续需要时根据生成的 pkl 文件重新计算。
