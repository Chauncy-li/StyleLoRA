"""Data structures for the primary v2 straight-scene split pipeline.

Compared with the legacy schema, v2 keeps the same three straight-scene buckets
and hard style labels, then adds:

- sample-quality fields
- memory-eligibility fields
- scene-specific continuous behavior axes
- auxiliary condition labels for offline analysis

The saved sidecar payload stays numpy-friendly so the existing indexing and
analysis scripts can keep their output format unchanged.
"""
...
STYLE_SCENE_SPLIT_V2_SCHEMA_VERSION = 5
...
DENSITY_LEVEL_NAME_TO_ID = {
    "unknown": 0,
    "sparse": 1,
    "medium": 2,
    "dense": 3,
}
DENSITY_LEVEL_ID_TO_NAME = {value: key for key, value in DENSITY_LEVEL_NAME_TO_ID.items()}

SPEED_REGIME_NAME_TO_ID = {
    "unknown": 0,
    "slow": 1,
    "urban": 2,
    "suburban": 3,
...
SPEED_REGIME_ID_TO_NAME = {value: key for key, value in SPEED_REGIME_NAME_TO_ID.items()}

CURVATURE_LEVEL_NAME_TO_ID = {
    "unknown": 0,
    "low": 1,
    "mild": 2,
    "high": 3,
}
CURVATURE_LEVEL_ID_TO_NAME = {value: key for key, value in CURVATURE_LEVEL_NAME_TO_ID.items()}

STYLE_PERFORMANCE_AXIS_BY_BUCKET: Dict[str, Tuple[str, str, str]] = {
    "straight_free_drive": (
        "speed_preference",
...
def style_axis_names_for_scene(scene_bucket: str) -> Tuple[str, str, str]:
    """Return the scene-specific continuous behavior axis names."""

    return STYLE_PERFORMANCE_AXIS_BY_BUCKET.get(scene_bucket, STYLE_PERFORMANCE_AXIS_BY_BUCKET["none"])


def _float_or_default(value: float | None, default: float = -1.0) -> np.ndarray:
    """Encode optional scalar values as stable numpy payloads."""
...
class StyleSceneSplitResultV2:
    """Single-sample v2 split result."""
    ...
    quality_score: float
    quality_reason: str
    quality_vec: np.ndarray
    style_performance_vec: np.ndarray
    style_performance_confidence: float
    condition_density_level: str
    condition_speed_regime: str
    condition_curvature_level: str

    def to_numpy_dict(self) -> Dict[str, np.ndarray]:
        """Serialize the result to the unchanged numpy sidecar format."""

        return {
            "style_scene_split_schema_version": np.array(STYLE_SCENE_SPLIT_V2_SCHEMA_VERSION, dtype=np.int64),
            ...
            "sample_quality_valid": np.array(int(self.sample_quality_valid), dtype=np.int64),
            "memory_eligible": np.array(int(self.memory_eligible), dtype=np.int64),
            "quality_score": np.array(self.quality_score, dtype=np.float32),
            "quality_reason": np.array(self.quality_reason),
            "quality_vec": np.asarray(self.quality_vec, dtype=np.float32),
            "style_performance_vec": np.asarray(self.style_performance_vec, dtype=np.float32),
            "style_performance_confidence": np.array(self.style_performance_confidence, dtype=np.float32),
            "condition_density_level": np.array(DENSITY_LEVEL_NAME_TO_ID[self.condition_density_level], dtype=np.int64),
            "condition_speed_regime": np.array(SPEED_REGIME_NAME_TO_ID[self.condition_speed_regime], dtype=np.int64),
            "condition_curvature_level": np.array(CURVATURE_LEVEL_NAME_TO_ID[self.condition_curvature_level], dtype=np.int64),
        }
