...
1. `python -m research_v1.stylization command build --base-index-path <split_index.jsonl>` ->
   `v6_direct_axis_conditions.jsonl`, `v6_style_command_spec.json`.
2. `python -m research_v1.stylization command validate` -> interface/mask contract.
3. `python -m research_v1.stylization command router_audit` -> offline-vs-two-gate agreement audit.
4. `python -m research_v1.stylization command support_audit` -> rho endpoint support audit.
...
The `command sweep_evaluation` stage consumes a model-agnostic JSONL. A
row must contain:
...
`research_v1.execution.evaluation.evaluate_checkpoints`.
...
`research_v1.execution.evaluation.run_closed_loop_suite` to
