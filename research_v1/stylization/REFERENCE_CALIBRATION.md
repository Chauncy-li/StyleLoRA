...
The `reference candidates` stage reads
...
The `reference normalization` stage fits q05/q95 independently for each
...
Validation/test use `reference apply_normalization` with that
frozen train model.

## 3. Conditional percentile calibration

The `reference rank` stage estimates
...
Validation/test use `reference apply_rank`; they do
