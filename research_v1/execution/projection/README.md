...
```bash
python -m research_v1.execution.projection.build_stats
```

Apply those stats to the val split:

```bash
python -m research_v1.execution.projection.run_projection
```

Validate the resulting projection dataset:

```bash
python -m research_v1.execution.projection.validate
```
