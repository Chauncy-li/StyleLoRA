...
PRESET_DEFAULT_EXPERIMENT_NAMES = {
    ...
    EXPERIMENT_PRESET_V6_SIGNED_ROUTER_NCQT_STAGE_B3_SCENE_AXIS_EXECUTOR: "continuous_style_v6_signed_router_ncqt_stage_b3_scene_axis_executor",
}
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "the A3.7 free-drive axis-temporal adapter, or the B3 complete "
            "scene-axis temporal adapter."
        ),
    )
    parser.add_argument(
...
def _apply_experiment_preset(
    ...
    *,
    ...
    elif preset == EXPERIMENT_PRESET_V6_SIGNED_ROUTER_NCQT_STAGE_B3_SCENE_AXIS_EXECUTOR:
        # B3 retains the complete A3.8 optimization/data contract, but replaces
        # car-follow's historical global acceleration fallback with three
        # independent, fixed-bank car-follow axis executors.  Free-drive names
        # and weights remain A3.8-compatible for safe checkpoint warm-starting.
        preset_overrides = {
            "condition_field": "style_value_condition",
            "base_style_condition_dim": 12,
            "style_condition_feature_set": "global_only",
            "style_condition_encoder": "axis_router_v2_signed",
            "axis_router_token_dim": 64,
            "signed_router_injection_mode": "ego_scene_axis_temporal_residual",
            "signed_router_diffusion_gate_mode": "free_drive_terminal_only",
            "signed_router_terminal_t_max": 0.0011,
            "kinematic_ego_basis_count": 6,
            "preference_axis_reference_mode": "normal_neighbor",
...
def _build_args() -> argparse.Namespace:
    ...
    if args.signed_router_diffusion_gate_mode == "free_drive_terminal_only":
        if args.signed_router_injection_mode not in {
            "ego_axis_temporal_residual",
            "ego_scene_axis_temporal_residual",
        }:
            raise ValueError(
                ...
                "an axis-temporal injection mode"
            )
...
def _assert_runtime_files_are_patched(args: argparse.Namespace) -> None:
    ...
    if args.style_condition_encoder == "axis_router_v1":
        ...
        elif args.signed_router_injection_mode == "ego_scene_axis_temporal_residual":
            decoder_markers.extend(
                [
