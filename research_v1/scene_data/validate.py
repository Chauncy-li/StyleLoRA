"""Validate the v2 scene split and its added eligibility layers.

In addition to hard-label ordering, this checks consistency among
``quality_valid``, ``split_valid``, and ``memory_eligible``; monotonic
agreement between continuous style values and hard labels; and confinement
of memory subsets to the predefined primary scene buckets.
"""
...
def _ordering_triplet(values: Dict[str, float | None], ascending: bool) -> Dict[str, object]:
    """Check the expected aggressive/normal/conservative ordering."""
...
def _mean_axis(records: List[Dict[str, object]], axis_index: int) -> float | None:
    """Compute one continuous-style axis mean for a sample group."""
...
def main():
    ...
    # Retain the legacy hard-label ordering check on memory-eligible samples.
