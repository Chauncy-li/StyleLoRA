...
def main() -> None:
    ...
    parser.add_argument("--config-root", default="/home/lisw/programs/Nuplan-Diffusion-Baseline/baseline/config")
    ...
    search = f"[pkg://nuplan.planning.script.config.common,pkg://nuplan.planning.script.experiments,file://{args.config_root},file://{repository / 'research_lora' / 'config'}]"
    records = []
    for rho in (float(item) for item in args.rhos.split(",")):
        ...
        command = [sys.executable, "-m", "nuplan.planning.script.run_simulation", f"+simulation={args.challenge}",
                   ...
                   f"planner.lora_diffusion_planner.config.lora_aggressive_adapter={args.aggressive_adapter}",
                   f"planner.lora_diffusion_planner.config.lora_conservative_adapter={args.conservative_adapter}",
