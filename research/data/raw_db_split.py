"""Utilities for splitting raw nuPlan DB logs before cache generation.

中文说明：
这个模块负责在生成 .npz cache 之前，先把完整 raw DB 名单切成
“可用于 cache/训练的数据”和“只用于 closed-loop simulation test 的数据”。
它只写 JSON/YAML 名单，不调用 DataProcessor，也不生成 .npz。

The split happens at the raw log/session level, before DataProcessor creates
NPZ cache samples.  This keeps closed-loop simulation logs out of the training
cache instead of trying to repair overlap after cache generation.
"""
...
def materialize_split(
    *,
    ...
    if write_filters:
        ...
        common_header = (
            "Generated by research.data.raw_db_split.\n"
