"""style_scene_split v2 的 raw scenario 构建器。

这个文件的职责是：
- 直接从原始 NuPlan scenario 构建 style cache / sidecar；
- 使用 v2 splitter 生成增强版结果；
- 以 memory_eligible 而不是 split_valid 作为新的记忆库准入标准；
- 保持与原始 raw_scenario_builder.py 并行，不影响旧流程。
"""
...
from research._runtime import DEFAULT_NUM_WORKERS
...
def _process_scenario_chunk_v2(worker_payload: Dict[str, object]) -> Dict[str, object]:
    """并行 worker 使用的 v2 构建函数。

    这里单独定义，是为了避免父类的 worker 仍然实例化旧版 builder。
    """
...
class RawScenarioStyleSceneSplitBuilderV2(RawScenarioStyleSceneSplitBuilder):
    """raw scenario 的 v2 构建器。

    相比旧版，主要有三点差异：
    1. 使用 StyleSceneSplitterV2；
    2. 记忆库准入使用 memory_eligible；
    3. index 中额外记录质量层、条件标签层和连续风格向量。
    """
    ...
    def __init__(
        ...
        # 明确替换为 v2 splitter，保证其余基础设施不受影响。
    ...
    def process_scenarios(
        ...
        """处理 raw scenario 列表并输出增强版 summary。"""
    ...
    def _process_scenarios_parallel(
        ...
        这里复用父类的分块逻辑，但把 worker 函数换成 v2 版本。
        """
    ...
    def process_single_scenario(self, scenario: object) -> Tuple[str, str]:
        """处理单个 raw scenario，并写出 v2 style cache 与 sidecar。"""
    ...
    def rebuild_global_index(self) -> Dict[str, object]:
        ...
        和旧版不同：
        - subset_lists 只按 memory_eligible 构建；
        - 额外统计 quality_valid 和 memory_eligible 两层数量。
        """
    ...
    def _compose_index_record(self, filename: str, sidecar_path: str, sidecar_data) -> Dict[str, object]:
        ...
        # planner_cache_path 在 raw 模式下实际上对应生成后的 style_cache_path。
