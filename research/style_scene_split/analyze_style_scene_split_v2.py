"""style_scene_split v2 的分析脚本。

这个脚本的重点不是替代旧版 analyze，而是在 v2 的新字段上做补充：
- 统计样本质量层；
- 统计 memory_eligible 覆盖率；
- 汇总连续风格向量在各 subset 中的分布；
- 汇总主场景桶在不同条件标签下的风格分布。
"""
...
def _safe_float_list(values: object) -> List[float]:
    """把 index 中的 list 字段安全转换为 float list。"""
