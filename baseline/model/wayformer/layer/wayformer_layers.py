...
class TrainableQueryProvider(nn.Module, QueryProvider):
    ...
    该类 TrainableQueryProvider 是一个可学习的查询向量提供器，常用于Perceiver IO模型中。其功能如下：
        - 初始化：构造函数创建了一个形状为 (num_queries, num_query_channels) 的可学习参数 _query，并按正态分布初始化。
        - 参数初始化：使用 normal_ 方法对 _query 进行均值为0、标准差为 init_scale 的初始化。
        - 获取通道数：返回查询向量的通道维度。
        - 前向传播：将 _query 扩展一个批次维度后返回，供后续模型使用。
    """
    ...
    def _init_parameters(self, init_scale: float):
        with torch.no_grad():
            self._query.normal_(0.0, init_scale)    # 按照指定标准差和均值进行正态分布初始化
...
class RotaryPositionEmbedding:
    """通过在多头注意力机制中使用正弦函数旋转查询和键向量，直接将位置信息编码到注意力机制中"""
...
def init_parameters(module, init_scale):
    for m in module.modules():
        if isinstance(m, nn.Linear):
            # 若为线性层（nn.Linear），则对其权重使用均值为0、标准差为init_scale的正态分布初始化，偏置初始化为0
            m.weight.data.normal_(mean=0.0, std=init_scale)
            if m.bias is not None:
                m.bias.data.zero_()
        elif isinstance(m, nn.Embedding):
            # 若为嵌入层（nn.Embedding），则对其权重使用均值为0、标准差为init_scale的正态分布初始化
            m.weight.data.normal_(mean=0.0, std=init_scale)
...
class CrossAttention(nn.Module):
    def __init__(
            ...
            num_q_input_channels: int,      # 查询向量的通道数（维度）
            num_kv_input_channels: int,     # 键/值向量的通道数
            num_qk_channels: Optional[int] = None,
            num_v_channels: Optional[int] = None,       # 值向量的通道数，默认等于 num_qk_channels
            max_heads_parallel: Optional[int] = None,   # 最多并行处理的注意力头数量（默认为 num_heads）
            causal_attention: bool = False,
            dropout: float = 0.0,
            qkv_bias: bool = True,      # Q/K/V 投影是否使用偏置项
            out_bias: bool = True,      # 输出投影是否使用偏置项
    ):
        """Pre-layer-norm cross-attention (see `MultiHeadAttention` for attention details).
        让输入的查询向量 x_q 去关注来自另一个上下文的键/值向量（x_kv 或者 x_kv_prefix + x_q）
        在 Perceiver AR（Autoregressive）等模型中，x_kv_prefix 和 x_q 的拼接使得当前查询可以“看到”前面已生成的内容"""
        super().__init__()
...
# -----------------------------------------------------------
# 编码器和解码器来源于 Wayformer，它是一个基于 Perceiver IO 架构 的多智能体轨迹预测模型。其核心思想是：
# 使用少量可学习的 latent 向量对输入场景进行注意力聚合（通过编码器）
# 再通过解码器从 latent 向量中提取目标对象未来的轨迹信息
# -----------------------------------------------------------
class PerceiverEncoder(nn.Module):
    def __init__(
            self,
            num_latents: int,               # latent 向量的数量 N
            num_latent_channels: int,       # 每个 latent 向量的维度 D
            num_cross_attention_heads: int = 4,         # cross-attention 使用的 head 数量 H
            num_cross_attention_qk_channels: Optional[int] = None,  # cross-attention 中 query/key 的通道数，默认等于 num_latent_channels
            num_cross_attention_v_channels: Optional[int] = None,   # value 的通道数，默认等于 num_qk_channels
            num_cross_attention_layers: int = 1,        # cross-attention 层数 Lc
            first_cross_attention_layer_shared: bool = False,       # 是否复用第一个 cross-attention 层的参数
            cross_attention_widening_factor: int = 4,   # cross-attention 内部 MLP 的扩展倍数
            num_self_attention_heads: int = 4,          # self-attention 的 head 数量 H'
            num_self_attention_qk_channels: Optional[int] = None,   # self-attention 中 query/key 的通道数，默认为 num_latent_channels
            num_self_attention_v_channels: Optional[int] = None,    # self-attention 中 value 的通道数，默认为 num_latent_channels
            num_self_attention_layers_per_block: int = 1,   # 每个 self-attention block 中的层数
            num_self_attention_blocks: int = 2,         # self-attention block 数量 B
            first_self_attention_block_shared: bool = False,    # 是否复用第一个 self-attention block 的参数
            self_attention_widening_factor: int = 4,    # self-attention 中 MLP 的扩展倍数
            dropout: float = 0.1,       # attention 矩阵中的 dropout 概率
            residual_dropout: float = 0.0,  # 残差连接中的 dropout 概率
            init_scale: float = 0.02,       # 初始化参数的标准差
    ):
        """Generic Perceiver IO encoder.
        一个基于 Transformer 的编码器结构，其核心思想是使用 少量可学习的 latent 向量（query）来对输入数据进行注意力聚合，
        从而实现高效的特征提取。它结合了 交叉注意力（cross-attention）与自注意力（self-attention），适用于各种任务如图像、文本或轨迹建模

        Input (x): shape (B, M, C)
        ↓
        Latent Pool (learnable): shape (N, D)
        ↓
        Cross-Attention Layer(s): Q ∈ (N, D), K/V ∈ (M, C) → Output ∈ (N, D)
        ↓
        Self-Attention Block(s): Latent 之间交互 → 更强的上下文感知能力
    ...
    def extra_cross_attention_layer(self):
        # 如果需要多层交叉注意力，并且第一层不共享权重，返回 True （也就是新生成一层）
    ...
    def forward(self, x, pad_mask=None, return_adapted_input=False):
        ...
        x_adapted = x
        # x_latent 是从输入中提取出的少量潜在向量，用于表示整个输入场景的核心特征。它相当于一种“注意力摘要”或“记忆池”
        x_latent = self.latent_provider()   # 可学习的潜在查询，维度是 (num_queries, num_query_channels)
