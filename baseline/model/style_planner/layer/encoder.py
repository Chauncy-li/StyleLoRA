...
功能：
1. 编码邻居车辆、静态障碍物、车道/路线信息；
2. 进行多源特征融合，输出供扩散解码器使用的上下文表示。
"""
...
class Encoder(nn.Module):
    ...
    def forward(self, inputs):
        ...
        encoding_pos_result[~encoding_mask] = encoding_pos  # 填充有效部分
        # 将位置编码加到特征上
...
class AgentFusionEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        # 设置智能体类型标识：[1,0,0] 表示邻居智能体
        pos[..., -3:] = 0.0
        pos[..., -3] = 1.0

        B, P, V, _ = x.shape
        # 生成掩码：判断哪些位置是全 0（无效填充）
        ...
        # 转置为 (B*P, D, V) 以便在时间维度上进行混合
        x = x.permute(0, 2, 1)
        # 时间混合：跨时间步交换信息
        x = self.token_pre_project(x)
        # 转置回 (B*P, V, D)
        x = x.permute(0, 2, 1)
        # 通过多个 MixerBlock 深度处理
        ...
        type_embedding = self.type_emb(neighbor_type)  # 类型嵌入
        x = x + type_embedding
