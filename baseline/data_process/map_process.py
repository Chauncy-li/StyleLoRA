...
from baseline.data_process.utils import vector_set_coordinates_to_local_frame
...
def map_process(route_roadblock_ids, anchor_ego_state, coords, traffic_light_data, speed_limit, lane_route, map_features, max_elements, max_points):
    ...
    for feature_name in map_features:
        if feature_name == "LANE":
            ...
            # --- [修改开始] 新增 route_lanes_mask 初始化 ---
            route_lanes_mask = np.zeros((max_elements["ROUTE_LANES"], vector_map_lanes.shape[-2]), dtype=np.bool_)
            # --- [修改结束] ---
            ...
            for i in range(len(lane_on_route)):
                if lane_on_route[i] == True:
                    ...
                    # --- [修改开始] 填充 route_lanes_mask ---
                    route_lanes_mask[loc] = avails[i]
                    # --- [修改结束] ---
                    loc += 1
    ...
    vector_map_output = {'lanes': vector_map_lanes,
                         'lanes_mask': avails,  # 新增: lane mask
                         ...
                         'route_lanes_mask': route_lanes_mask,  # 新增: route lane mask
