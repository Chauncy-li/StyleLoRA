"""Server-aligned path configuration for research_lora_2.

research_lora_2 的服务器路径对齐配置：环境变量名与 research_v1/paths.py
完全一致，保证本地与服务器（/mnt/mydata/lishangwen/...）运行时可互相切换。

任何 research_lora_2 CLI 的默认数据/输出路径都应从这里取，避免硬编码路径。
"""
...
RESEARCH_ROOT = REPO_ROOT / "research_lora_2"
DEVKIT_ROOT = REPO_ROOT / "nuplan-devkit"

# ---- 服务器路径（与 research_v1/paths.py 环境变量名保持一致）----
...
# research_lora 的标准 manifest 目录约定：按拆分输出 train/val/test.jsonl
